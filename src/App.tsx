...
const Sidebar = ({ 
  ...
  return (
    ...
    >
      ...
      {/* Sync Panel - SUPPRIMÉ */}
      {/* Le panneau de synchronisation n'est plus nécessaire car nous utilisons uniquement Supabase */}
    </div>
  );
};
...
const AuthenticatedApp: React.FC = () => {
  ...
  const [isDarkMode, setIsDarkMode] = useState(() => {
    return localStorage.getItem('calendrcar-theme') === 'dark';
  });
  ...
  useEffect(() => {
    // Chargement initial simple depuis Supabase
    const loadInitialData = async () => {
      try {
        ...
        console.log('✅ Données chargées:', loadedVehicles.length, 'véhicules,', loadedReservations.length, 'réservations');
        ...
        // Marquer la fin du chargement initial même en cas d'erreur
        setIsInitialLoad(false);
      }
    };
    ...
    const handleOnline = () => {
      setIsOnline(true);
      console.log('🌐 Connexion en ligne rétablie');
    };
  ...
  useEffect(() => {
    ...
    if (reservations.length > 0) {
      ...
      const saveReservations = async () => {
        try {
          if (!navigator.onLine) {
            console.log('📴 Hors ligne - sauvegarde différée');
            showNotification('📴 Hors ligne - les modifications seront sauvegardées quand la connexion sera rétablie', 'warning');
            return;
          }
          
          await supabaseService.saveReservations(reservations);
          console.log('✅ Réservations sauvegardées dans Supabase');
          
        } catch (error) {
          console.error('❌ Erreur sauvegarde réservations:', error);
          showNotification('Erreur de sauvegarde des réservations', 'error');
        }
      };
      saveReservations();
    }
  }, [reservations, isInitialLoad]);
    ...
    if (vehicles.length > 0) {
      ...
      const saveVehicles = async () => {
        try {
          ...
          console.log('✅ Véhicules sauvegardés dans Supabase');
          
        } catch (error) {
          console.error('❌ Erreur sauvegarde véhicules:', error);
          showNotification('Erreur de sauvegarde des véhicules', 'error');
        }
      };
      saveVehicles();
    }
